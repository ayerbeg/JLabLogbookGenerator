...
per log line). Supported \code{type} values:
...
\midrule
\code{text}   & Free-text input. \\
\code{select} & Dropdown. Requires an \code{options} array; \code{default} is optional (falls back to the first option). \\
\code{date-now} & Text input with a ``Set Date'' button. Fills the field as \code{DD-MonthName-YYYY} from the system clock, then disables itself and turns green. State persists across reloads. \\
\bottomrule
\end{tabularx}
\end{center}

Add, remove, or reorder entries freely --- the build script generates the
form, save/load, generated-HTML output, and import parsing for each one
automatically. No \code{sheet\_column} is needed; these fields are not sent to
Google Sheets.
...
\begin{itemize}
    ...
    \item \code{sheet\_name} --- the exact name of the tab (sheet) within your
          Google Spreadsheet that rows should be written to. The tab must
          already exist; the script does not create it. Defaults to
          \code{"Sheet1"} if omitted.
...
\begin{warningbox}
\textbf{Why \code{sheet\_name} matters:} a Google Spreadsheet can contain
multiple tabs. The Apps Script targets a tab by name
(\code{getSheetByName}), not by ``whichever tab is active'' --- a Web App
triggered by an external HTTP request has no browser session, so there is no
``active'' tab in that context. Earlier versions of this script used
\code{getActiveSheet()}, which silently falls back to whichever tab is first
by position --- renaming a tab does \textbf{not} change its position, so this
could send data to the wrong tab without any error. If you have multiple
tabs, always set \code{sheet\_name} explicitly and rebuild.
\end{warningbox}
...
\code{type}         & yes & \code{text}, \code{textarea}, \code{checkbox} & Input type. Use \code{textarea} for Comments, \code{checkbox} for yes/no flags. \\
...
\code{placeholder}  & no  & any string & Placeholder text in the input. Ignored for \code{checkbox}. \\
\code{js\_key}       & no  & valid JS identifier & Overrides the auto-derived payload key name. See below. \\
\code{\_comment}    & no  & any string & Ignored by build script. For your notes. \\
\bottomrule
\end{tabularx}
\end{center}

\subsection*{\texttt{type: "checkbox"} example}
...
is sent to Google Sheets as a native \code{TRUE}/\code{FALSE} value (not a
string), and displays as ``Yes'' / ``No'' in the generated logbook HTML table.
Round-trips correctly through Import from Previous HTML. Use this pattern for
any yes/no flag --- a good-run marker, a ``replay OK'' checkbox, etc.
...
\subsection*{The \texttt{js\_key} property}

By default, the build script derives the payload key sent to Google Sheets by
stripping a leading \code{run-} from \code{key} and camelCasing the remainder
--- e.g.\ \code{run-hms-p} $\to$ \code{hmsP}. For most fields this is fine.
Four fields don't follow that pattern and use an explicit override:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\code{key} & Auto-derived (unused) & \code{js\_key} override actually used \\
\midrule
\code{run-number} & \code{number} & \code{runNumber} \\
\code{run-start}  & \code{start}  & \code{startTime} \\
\code{run-stop}   & \code{stop}   & \code{stopTime} \\
\code{run-length} & \code{length} & \code{totalTime} \\
\bottomrule
\end{tabular}
\end{center}

If you add a new field whose desired payload name doesn't mechanically match
its \code{key}, add \code{"js\_key": "yourName"} to that field's entry.
Otherwise, omit it and the default derivation is used.
