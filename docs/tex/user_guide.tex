...
\begin{itemize}
    \item \textbf{Shift} --- dropdown: Owl, Day, or Swing.
    \item \textbf{Shift Date} --- click \textbf{Set Date} to fill it
          automatically as \code{DD-MonthName-YYYY} (e.g.\
          \code{04-August-2026}). The button turns green and disables itself
          after use, so it is clear the date has already been set. This state
          is remembered even if you reload the page. You can still type
          directly into the field to correct it.
    \item \textbf{Shift Leader} --- free text.
    \item \textbf{Target Operator} --- free text.
\end{itemize}
