...
\code{js\_key(field)} takes a field's config dict (not just the CSS class)
and returns the JS object key used in the payload sent to
\code{saveToLocalStorage}, \code{loadFromLocalStorage}, \code{generateHTML},
and the Apps Script row array:
...
If the field's config has an explicit \code{"js\_key"} property, that value
is returned verbatim instead --- this is required for fields whose payload
name doesn't mechanically derive from the CSS class:
...
Four fields in the default config use this override (\code{run-number}
$\to$ \code{runNumber}, \code{run-start} $\to$ \code{startTime},
\code{run-stop} $\to$ \code{stopTime}, \code{run-length} $\to$
\code{totalTime}) so the build output matches a naming convention already in
use elsewhere. Add \code{js\_key} to any new field whose auto-derived name
isn't the one you want.

\begin{warningbox}
\textbf{Past bug, now fixed:} \code{extractRunData()} used to be generated by
two independent code paths --- \code{build\_extract\_run\_data()} (computed
but never actually interpolated into the template --- dead code) and a
second inline copy inside the Sheets-enabled branch of \code{build\_html()}
(the one that actually shipped). The two drifted out of sync at one point,
causing a payload key mismatch that silently dropped four columns when
writing to Google Sheets. The dead function has been removed; there is now
exactly one place that generates \code{extractRunData()}. If you need to
change what gets extracted per run, there is only one function to edit ---
search for \code{EXTRACT\_PLACEHOLDER} inside \code{build\_html()}.
\end{warningbox}
...
% ============================================================
\chapter{Shift Header Fields}
% ============================================================

Distinct from \code{run\_fields}, the \code{shift\_header} config section
defines fields shown once above the shift log entries (Shift type, Shift
Date, Shift Leader, Target Operator by default). These are generated by
\code{build\_shift\_header\_form()} and \code{build\_shift\_header\_js()},
which together produce the form HTML, the \code{setShiftDate()} function, and
the save/load/generate/clear/import fragments --- following the same pattern
as the Quick Links functions.

Supported \code{type} values: \code{text}, \code{select} (requires
\code{options}), \code{date-now} (adds a self-disabling ``Set Date'' button,
formats as \code{DD-MonthName-YYYY}).

\begin{warningbox}
\textbf{Naming pitfall to avoid:} the generated JS in
\code{build\_shift\_header\_js()} declares local variables prefixed with
\code{\_} (e.g.\ \code{const \_shiftType = ...}) specifically to avoid
colliding with the browser's automatic global reference to any element with
a matching \code{id} attribute (\code{window.shiftType} would otherwise
resolve to the \code{<select>} element itself, not its value, if a
same-named unprefixed variable were used inside a template literal). This bug
shipped once (fixed since) and produced \code{[object HTMLSelectElement]} in
the generated output instead of the selected value. Keep the \code{\_} prefix
...
(multi-line), \code{checkbox} (boolean flag).

\section{\texttt{checkbox} Implementation Reference}

\code{checkbox} fields are handled as a special case in every generator
function that touches \code{run\_fields}, since they read \code{.checked} (a
boolean) rather than \code{.value} (a string):

\begin{center}
\begin{tabularx}{\linewidth}{@{}lX@{}}
\toprule
\textbf{Function} & \textbf{Checkbox handling} \\
\midrule
\code{build\_add\_run\_entry\_fields()} &
    Emits \code{<input type="checkbox">} instead of a text input;
    \code{placeholder} and \code{readonly} styling are skipped. \\
\code{extractRunData} (inline, inside \code{build\_html()}) &
    Sends \code{item.querySelector('.key').checked} (boolean) instead of
    \code{.value}. \\
\code{build\_generate\_html\_run\_table()} &
    Converts to \code{'Yes'} / \code{'No'} text for display in the generated
    logbook HTML table. \\
\code{build\_save\_run\_entries()} &
    Saves \code{.checked} (boolean) to localStorage. \\
\code{build\_load\_run\_entries()} &
    Restores with \code{.checked = !!run.fieldName} instead of
    \code{.value = ...}. \\
\code{build\_import\_run\_entries()} &
    Parses back from the \code{'Yes'}/\code{'No'} table text:
    \code{.checked = cellText === 'Yes'}. \\
\code{build\_apps\_script()} row construction &
    Uses \code{typeof run.fieldName === 'boolean' ? run.fieldName : ''}
    rather than \code{run.fieldName || ''} --- the \code{||} pattern used for
    text fields would incorrectly collapse \code{false} to an empty string,
    silently turning an explicit ``No'' into a blank cell. \\
\bottomrule
\end{tabularx}
\end{center}

If you add another non-text field type, expect to touch all seven of these
spots --- search for \code{f["type"] == "checkbox"} in \code{build.py} to
find every one.

\section{Adding \texttt{select} (Dropdown) as a Further Example}
...
\begin{enumerate}
    ...
    \item The save/load/export machinery reads \code{.value} from the DOM
          ...
          expose \code{.value} the same as inputs, so \code{select} needs
          far less special-casing than \code{checkbox} did.
\end{enumerate}
